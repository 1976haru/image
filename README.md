...
SDXL 모델은 프로그램에서 처음 사용할 때 자동 다운로드됩니다. 첫 다운로드는 시간이 오래 걸릴 수 있고, 인터넷 연결과 저장 공간이 필요합니다. 모델 다운로드 실패, CUDA 오류, 메모리 부족, 추론 오류가 발생하면 해당 출력은 `자연 배경 확장`으로 전환하고, 이것도 실패하면 `블러 배경`으로 전환합니다. 실패 원인은 로그와 작업 JSON에 기록됩니다.
