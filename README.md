...
`RUN.bat`을 실행하면 프로젝트나 채널을 먼저 만들지 않아도 되는 간편 제작 화면이 열립니다.

1. JSON/TXT를 불러오거나 큰 입력창에 가사·주제 또는 이미지 프롬프트를 입력합니다.
2. 입력 방식과 후보 수(1/4/5)를 고르고 `이미지 만들기`를 누릅니다.
3. 완성되는 후보 썸네일을 선택합니다. 취소해도 이미 완성된 후보는 보존됩니다.
4. 제목·부제·채널명, 배치, 폰트, 색상과 위치를 편집합니다.
5. 출력 폴더를 선택하고 `커버 저장`을 누릅니다.

완성본은 정확한 1400×1400 RGB JPG로, 글자 없는 생성 원본은 원래 해상도의 PNG로
별도 보존됩니다. 1024×1024 배경은 일반 LANCZOS 리사이즈로 1400×1400에 맞추며 AI
업스케일 또는 디테일 복원으로 표시하지 않습니다. 입력, 후보와 후보별 문구 설정은 내부
프로젝트에 자동 저장됩니다. 기존 기능은 `고급 프로젝트 화면` 버튼으로 계속 사용할 수
있습니다.

기존 커버 글자 제거와 AI 배경 확장은 삭제되지 않았으나 **시험 기능: 결과 확인 필요**입니다.
그 품질은 간편 커버 제작 흐름의 완료 조건이 아니며, 해결된 기능으로 간주하지 않습니다.
